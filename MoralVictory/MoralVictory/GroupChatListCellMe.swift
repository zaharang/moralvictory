...
class GroupChatListCellMe: GroupChatListCell {
    ...
    override func setupLayoutConstraint(withTalkItem talkItem: Talk) {
        ...
        let contentStringExpectRect = NSString(string: talkItem.content).boundingRect(with: CGSize(width: frame.width - receivedTimeLabel.frame.width, height: CGFloat.greatestFiniteMagnitude/2), options: [NSStringDrawingOptions.usesLineFragmentOrigin], attributes: [NSAttributedStringKey.font:talkLabelFont], context: nil)
        ...
        if contentStringExpectRect.height < 17 {
            ...
            receivedTimeLabel.autoPinEdge(.left, to: .left, of: talkLabelGroup, withOffset: 5)
        }
